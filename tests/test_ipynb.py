...
def test_entire_notebook_trailing_newline() -> None:
    ...
    expected = (
        ...
        "  },\n"
        '  "kernelspec": {\n'
        '   "display_name": "Python 3.8.10 64-bit (\'grey\': venv)",\n'
        '   "name": "python3"\n'
        "  },\n"
        '  "language_info": {\n'
        '   "name": "python",\n'
...
def test_cache_isnt_written_if_no_jupyter_deps_single(
    ...
    with open(nb) as src, open(tmp_nb, "w") as dst:
        dst.write(src.read())
    monkeypatch.setattr(
        "grey.jupyter_dependencies_are_installed", lambda verbose, quiet: False
    )
    result = runner.invoke(
    ...
    jupyter_dependencies_are_installed.cache_clear()
    monkeypatch.setattr(
        "grey.jupyter_dependencies_are_installed", lambda verbose, quiet: True
    )
    result = runner.invoke(
        main, [str(tmp_path / "notebook.ipynb"), f"--config={EMPTY_CONFIG}"]
    )
...
def test_cache_isnt_written_if_no_jupyter_deps_dir(
    ...
    with open(nb) as src, open(tmp_nb, "w") as dst:
        dst.write(src.read())
    monkeypatch.setattr(
        "grey.files.jupyter_dependencies_are_installed",
    ...
    jupyter_dependencies_are_installed.cache_clear()
    monkeypatch.setattr(
        "grey.files.jupyter_dependencies_are_installed", lambda verbose, quiet: True
    )
    result = runner.invoke(main, [str(tmp_path), f"--config={EMPTY_CONFIG}"])
    assert "reformatted" in result.output
...
def test_unable_to_replace_magics(monkeypatch: MonkeyPatch) -> None:
    src = "%%time\na = 'foo'"
    monkeypatch.setattr("grey.handle_ipynb_magics.TOKEN_HEX", lambda _: "foo")
    with pytest.raises(
